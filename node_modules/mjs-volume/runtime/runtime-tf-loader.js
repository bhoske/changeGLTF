    ...
    handleMesh: {
        value: function(entryID, description, userInfo) {
            ...
            for (var i = 0 ; i < primitivesDescription.length ; i++) {
				...
				//code added for line support /mbd
				if (primitiveDescription.primitive === WebGLRenderingContext.LINES)
				{
                    var checkPMI = mesh.id;
					var meshName = mesh.name;
					
					if(checkPMI.indexOf("pmi") != -1)
					{
						var getPMIDataType = meshName.split('_TN_'); //fill PMI data type from mesh
						if(getPMIDataType[1])
						{
							var getName = getPMIDataType[1].split('_TextPos_');
							if(getName[1])
							{
								var getTextPos = getName[1].split(',');
								
								var dirAB = vec3.createFrom(	getTextPos[3] - getTextPos[0], 
																getTextPos[4] - getTextPos[1],
																getTextPos[5] - getTextPos[2]);
														
								var dirAC = vec3.createFrom(	getTextPos[6] - getTextPos[0],
																getTextPos[7] - getTextPos[1],
																getTextPos[8] - getTextPos[2]);
								var dirAD = vec3.createFrom(0,0,0);							
								
								var dirAC1 = vec3.create(), dirAB1 = vec3.create(), dirAD1 = vec3.create();
								vec3.normalize(dirAB, dirAB1);
								vec3.normalize(dirAC, dirAC1);
								vec3.multiply(dirAB1, dirAC1, dirAD);
								vec3.normalize(dirAD, dirAD1);
								
								
								pmiDataTypeArray.push(getPMIDataType[0]);
								pmiNameArray.push(getName[0]); //fill PMI array from mesh
								pmiPositionArray[pmiPositionCnt] = new Array();
								pmiPositionArray[pmiPositionCnt][0] = dirAB1[0];
								pmiPositionArray[pmiPositionCnt][1] = dirAB1[1];
								pmiPositionArray[pmiPositionCnt][2] = dirAB1[2];
								pmiPositionArray[pmiPositionCnt][3] = dirAC1[0];
								pmiPositionArray[pmiPositionCnt][4] = dirAC1[1];
								pmiPositionArray[pmiPositionCnt][5] = dirAC1[2];
								pmiPositionArray[pmiPositionCnt][6] = dirAD1[0];
								pmiPositionArray[pmiPositionCnt][7] = dirAD1[1];
								pmiPositionArray[pmiPositionCnt][8] = dirAD1[2];
								pmiPositionArray[pmiPositionCnt][9] = getTextPos[0];
								pmiPositionArray[pmiPositionCnt][10] = getTextPos[1];
								pmiPositionArray[pmiPositionCnt][11] = getTextPos[2];
								pmiPositionCnt++;
							}
						}
					}
